...
export function createSEDO(options = {}) {
  ...
  backupProviders.set("table-datasets", {
    ...
    export: async () => {
      ...
      if (!store?.exportTableData) {
        return { format: "sdo-table-data", formatVersion: 1, exportedAt: new Date().toISOString(), datasets: [] };
      }

      const data = await store.exportTableData({ includeFormatting: true });

      // Fallback: if the tableStore index is missing/corrupted, try exporting by journal ids from navigation.
      // This prevents the common scenario: "structure restored, but table rows are missing" after restore.
      if (!data?.datasets?.length) {
        try {
          const nav = await api.exportNavigationState?.();
          const ids = Array.isArray(nav?.journals) ? nav.journals.map(n => n?.id).filter(Boolean) : [];
          const datasets = [];
          for (const journalId of ids) {
            const ds = await store.getDataset?.(journalId);
            const hasAny =
              (ds && ds.meta && Object.keys(ds.meta).length > 0) ||
              (ds && ds.records && Object.keys(ds.records).length > 0) ||
              (Array.isArray(ds?.order) && ds.order.length > 0);
            if (hasAny) datasets.push({ journalId, meta: ds.meta || {}, order: ds.order || [], records: ds.records || {} });
          }
          if (datasets.length) {
            return { ...data, datasets };
          }
        } catch (e) {
          // ignore fallback errors and return original export
        }
      }
