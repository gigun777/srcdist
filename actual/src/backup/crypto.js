...
export async function verifyIntegrity(bundle) {
  // ZIP v2 import constructs a BACKUP_FORMAT bundle on the fly.
  // It may not include legacy integrity payload. In that case we accept it.
  // If integrity is present, we strictly verify it.
  if (!bundle?.integrity?.payloadHashB64) return true;
