...
export async function exportAllZipV2({ sdoInst, zipTools, downloadBlob } = {}) {
  ...
  for (const j of journals) {
    ...
    // Prefer tableStore.getDataset (single source of truth)
    let ds = null;
...
export async function importAllZipV2({ sdoInst, zipTools, pickFile, forceTableRerender } = {}) {
  ...
  // Replace: clear storage best-effort
  if (mode === 'replace') {
    try {
  ...
  // Apply templates/settings/navigation directly to storage/runtime.
  // This avoids dependence on sdo.importBackup() which may be absent/unstable.
  ...
  // 5) journals: read each file and collect datasets (+ per-journal renderer settings)
  const datasets = [];
